...
// badOrderSeverity and classifyBadOrderRisk add their own presentation on top.
export type BadOrderBand =
  | "healthy"
  | "needs-attention"
  | "high-risk"
  | "risky";

export function badOrderBand(ratePct: number): {
  band: BadOrderBand;
  label: string;
} {
  ...
  if (ratePct >= 5)
    return { band: "needs-attention", label: "Needs attention" };
